          ...
          {aiFeatures.map((feature, i) => (
            <Reveal key={feature.title} delay={i * 0.08}>
              <motion.div
                ...
                className="service-card group relative overflow-hidden text-center"
              >
                <div className="service-card-icon mx-auto">
                  <feature.icon size={24} strokeWidth={1.5} />
                </div>
                <h3 className="mb-2 font-bold text-foreground transition-colors duration-300 group-hover:text-brand-primary">
                  {feature.title}
                </h3>
    ...
    <Reveal className="mt-16">
      <div className="relative mx-auto h-64 max-w-2xl sm:h-80">
        ...
        {nodes.map((node, i) => (
          <motion.div
            key={node.label}
            className="service-card absolute flex h-16 w-24 -translate-x-1/2 -translate-y-1/2 items-center justify-center !rounded-xl !p-0 text-xs font-semibold text-brand-primary"
            style={{ left: node.x, top: node.y }}
            animate={{
              boxShadow: [
                "0 2px 12px rgba(68,96,239,0.08)",
                "0 4px 20px rgba(68,96,239,0.14)",
                "0 2px 12px rgba(68,96,239,0.08)",
              ],
            }}
        ...
        <motion.div
          className="absolute left-1/2 top-1/2 flex h-20 w-20 -translate-x-1/2 -translate-y-1/2 items-center justify-center rounded-[20px] border border-brand-primary/20 bg-white text-sm font-bold text-brand-primary shadow-[0_4px_24px_rgba(68,96,239,0.12)]"
